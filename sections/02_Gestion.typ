...
- Desarrollar un *sistema capaz de gestionar y convertir distintos formatos* de datos genómicos (FASTQ, FASTA y anotaciones generadas con _Bakta_), ofreciendo un entorno único en el que realizar los procesos de ensamblaje y anotación.
...
Para el desarrollo de este proyecto, se ha adoptado una metodología ágil adaptada a las características de un proyecto individual. Se ha priorizado la flexibilidad en la organización para poder adaptarse a las necesidades del software y a la carga lectiva del curso. Por ello, se ha optado por *Scrumban*, una metodología híbrida que combina la estructura de _Scrum_ con la fluidez de _Kanban_.

Scrumban combina la planificación iterativa propia de _Scrum_ con la gestión visual del flujo de trabajo ofrecido por _Kanban_. El desarrollo se ha dividido en iteraciones de duración variable (sprints), donde se definen objetivos concretos a corto plazo. Al inicio de cada iteración se establecen las metas y al finalizar se evalúan los resultados obtenidos, ajustando la planificación en función del progreso y cambios en las prioridades y los requisitos.
...
Este enfoque permite un desarrollo incremental y flexible, facilitando la entrega continua de funcionalidades sin depender de ciclos cerrados rígidos, adaptándose a las necesidades del proyecto y al ritmo académico.


== Planificación
La planificación del proyecto se ha estructurado mediante un enfoque iterativo basado en sprints, tal y como se ha descrito en la sección de metodología. El proyecto se ha dividido en sprints de duración variable, con una planificación adaptada a la complejidad y necesidades de cada módulo.
...
Adicionalmente, la fase de desarrollo de la infraestructura también presentó una desviación relevante, debido a la complejidad asociada al despliegue del sistema y el servicio de conversión de datos genómicos mediante Docker y la incorporación de mecanismos de paralelización de tareas mediante la herramienta Celery.
...
El cálculo de los costes del proyecto tiene en cuenta varias partidas, cuyo importe depende en mayor o menor medida de la dedicación empleada.
...
#riesgo(
  "Complejidad de la generación de features",
