...
El *sistema web* es el núcleo de la aplicación, el encargado de gestionar la lógica de negocio. Este bloque incluye un backend que expone una API, encargada de procesar las peticiones del usuario, gestionando la persistencia de datos y ficheros y coordinando la ejecución de tareas. Para ello, el sistema dispone de:
...
El modelo de clases definido tiene como objetivo establecer una estructura modular y extensible, praparada para dar soporte al pipeline bioinformático, modelos predictivos y distintos mecanismos de notificación sin necesidad de rediseñar la arquitectura de persistencia del sistema.
...
Además, se permite la transición de `pending`a `failed` para contemplar errores producidos antes del inicio del procesamiento, como problemas durante el envío inicial de la tarea.
...
El adaptador invoca entonces al modelo de predicción correspondiente, que procesa las características de entrada y devuelve la predicción de resistencia. Esta respuesta se devuelve al adaptador que la envía de vuelta al backend, que se encarga de agregar los resultados de las ejecuciones y prepararlos para su visualización.
...
==== Patrones derivados del framework y la infraestructura
Estos patrones se han aplicado de forma implícita en el sistema, ya que vienen dados por las herramientas utilizadas.

/ *Modelo-Vista-Template (MVT)*: El sistema web se apoya en el patrón MVT proporcionado por _Django_, que organiza la aplicación en torno a modelos de datos, vistas que gestionan la lógica de negocio y templates para la presentación de la información al usuario. Este patrón facilita la separación de responsabilidades y mejora la mantenibilidad del código.

/ *Command*: El sistema de ejecución asíncrona basado en _Celery_ sigue el patrón Command, donde cada tarea se define como un comando que encapsula una acción (por ejemplo, lanzar un ensamblaje o consultar el estado de un proceso) y puede ser ejecutada por los workers de forma desacoplada del resto del sistema.

/ *Singleton*: Componentes como la configuración de _Django_ y la instancia de _Celery_ actúan como singletons, ya que se inician una sola vez y se reutilizan en todo el sistema.
...
/ *Adapter - Adaptadores para modelos predictivos*: Uno de los principales retos del sistema es la integración de modelos de predicción heterogéneos, con distintos formatos de entrada, configuraciones y salidas. Para abordar esto, se ha adoptado el patrón _Adapter_, creando una capa de adaptadores que actúan como intermediarios entre la lógica del sistema y los modelos de predicción. Cada adaptador se encarga de transformar las características generadas a partir de los resultados de anotación en el formato requerido por el modelo, así como de gestionar la carga de parámetros y la ejecución del modelo. Esto permite:
  - Incorporar nuevos modelos sin modificar la lógica principal del sistema, mediante la implementación de nuevos adaptadores.
...
- *Simplificación del flujo de trabajo*: se abstrae la complejidad del pipeline de análisis genómico, permitiendo su ejecución mediante interacciones sencillas como la subida de un archivo o la selección de opciones en la interfaz.
