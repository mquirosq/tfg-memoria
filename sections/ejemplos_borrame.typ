...
Para cambiar el formato de grandes cantidades de contenido, deben usarse las funciones ```Typst #set funcion(parametro:valor)``` (altera un parámetro siempre que se llame a la función ) y ```Typst #show funcion: it => {...}``` (reemplaza el resultado de una función completamente).
...
#figure(
  image("/memoria/figures/ejemplo.png", width: 70%),
...
#figure(
  raw(read("/memoria/code/java_example.java"), block: true, lang: "java"),
  caption: "Código Java",
)<cod:java>

#figure(
  raw(read("/memoria/code/html_example.html"), block: true, lang: "html"),
  caption: "Código HTML",
)<cod:html>

#figure(
  raw(read("/memoria/code/javascript_example.js"), block: true, lang: "javascript"),
